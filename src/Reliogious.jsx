...
const Reliogious = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Reliogious');
    XLSX.writeFile(workbook, 'Reliogious.xlsx');
  };

  // Filter entityBData where selectedOption is 'teacher'
  const filteredUsers = entityBData.filter(item => item.selectedOption === 'Religious Trainers');

  return (
    <div className='row'>
      <Admin_Dasbod/>
      <center className='fs-3 text-secondary'><h1>Religious Trainers</h1></center>
      ...
      <ul className='row'>
        {filteredUsers.map(item => (
          <div className='row m-3' key={item.id}>
            ...
            <div className='col-6 col-sm-2 col-lg-2 border border-info overflow-hidden'>{item.email}</div>
